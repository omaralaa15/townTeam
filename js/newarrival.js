...
xhr.open("GET", "../json/newarrival.json");
xhr.onreadystatechange = function () {
  if (xhr.readyState === 4 && xhr.status === 200) {
    data = JSON.parse(xhr.responseText);
    displayData(data.collection.newarrival);
  }
};
xhr.send();
...
function viewProduct() {
  ...
  cardId.forEach((card) => {
    card.addEventListener("click", () => {
      ...
      window.location.assign(`../html/singleNewarrival.html?id=${productId}`);
    });
  });
}
...
document.getElementById("black").addEventListener("click", () => {
  let filteredProducts = data.collection.newarrival.filter((product) =>
